...
Composite pre-test walkthrough for `HeterogeneousAdoptionDiD`, building on Tutorial 20's brand-campaign framing on a Design 1 (`continuous_at_zero`) panel variant:
...
- Upgrade to `did_had_pretest_workflow(aggregate="event_study")` on the multi-week panel: closes all three testable steps via QUG + joint pre-trends Stute + joint homogeneity Stute
