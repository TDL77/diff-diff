...
Full equation-level review (all equation/theorem/page pins below refer to the published version): `docs/methodology/papers/athey-imbens-2006-review.md`. Companion reviews for the deferred extensions: `callaway-li-oka-2018-review.md` (panel QTT / bootstrap validity machinery), `melly-santangelo-2015-review.md` (covariates), `ciaccio-2024-review.md` (staggered).
...
- Support check: treated pre-period outcomes outside the control pre-period range (Assumption 3.4 / Corollary 3.1) -> `UserWarning`; quantile effects remain point-identified only on the eq. (17) interior range.
...
*Interior-range guard (eq. 17 / Theorem 5.3):* `q_lower = ECDF_y10(min y00)`, `q_upper = ECDF_y10(max y00)`; requested quantiles outside the open interval `(q_lower, q_upper)` keep their point estimates (qte parity - `qte::CiC()` has no guard) but report NaN inference with a `UserWarning` (Theorem 5.3's asymptotics are interior-only).
...
- Covariates (paper Section 5.1 routes; Melly-Santangelo 2015 QR pipeline is the modern reference). `fit()` accepts no covariates; formula covariate terms raise `ValueError`.
