...
- **Note (rank-guard column-drop):** The rank-guarded IF inverse uses the point estimate's column-drop convention (pivoted QR on the equilibrated Gram), so the analytical SE equals the well-conditioned near-collinear limit (`se_ratio ≈ 1`) for StaggeredTripleDifference's per-pair OR bread (`_compute_did_panel`) and PS Hessian (`_compute_pscore`), with no minimum-norm divergence — see the CallawaySantAnna "rank-guarded IF standard errors" Note for the full statement.
