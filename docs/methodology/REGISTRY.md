...
- **Restrictions / unchanged from Wave E.1/E.2/follow-up:**
  ...
  - TwoStageDiD's analogous `finite_mask + design-subset + recompute n_psu/n_strata` pattern at `two_stage.py:567-601` is NOT yet adopted to Wave E.3's full-design retention — separate parity follow-up tracked in TODO.md. (No expected-divergence test ships in this PR: TwoStageDiD's always-treated handling at `two_stage.py:294-336` differs from SpilloverDiD's per-unit Omega_0 rank-deficiency check, so the divergence does not materialize on the standard SpilloverDiD warn-and-drop fixtures. The parity follow-up should add a targeted regression on a fixture that actually exercises the TwoStageDiD finite-mask path.)
