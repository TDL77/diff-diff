...
- Finite-sample refinement: **leave-one-out** modification of `τ̃_it` (Supp. App. A.9 — GAP).
- **⚠ Implementation deviation (PR-B reconciliation target):** the library's `_compute_auxiliary_residuals_treated` (`imputation.py`) currently uses the *observation-level* weighted mean `τ̃_g = Σ_{it∈G_g} v_it·τ̂*_it / Σ_{it∈G_g} v_it`, NOT the *unit-clustered* Eq. (8) above. They coincide for uniform within-group treated weights — the **unweighted** overall ATT / single-horizon event-study under the default `cohort_horizon` partition — but diverge for non-uniform `v_it`: **survey-weighted estimands** (`survey_design=` → even overall ATT), balanced / heterogeneity estimands (this `v_it` channel applies to *any* partition), or a unit contributing **multiple observations** to a group under the coarser `cohort` partition (under `horizon` each unit still has ≤1 obs per group, so its divergence is the `v_it` channel only). The simplification stays within Theorem 3's conservative-inference conditions (`σ²_τ ≥ 0` for any auxiliary model converging to a non-random limit), so SEs remain valid/conservative — it is simply not the excess-variance-minimizing weighting (Supp. App. A.8), so CIs can be marginally wider off the default path. The code docstring still labels the simplified form as "Equation 8" (this PR corrects the `REGISTRY.md` label; the docstring fix lands with the PR-B code change). **PR-B reconciles the implementation to the exact unit-clustered Eq. (8)** (per fix-code-to-match-theory) with a methodology regression test on a diverging case. Surfaced by the PR-A AI review (2026-06-06).
...
- [~] Auxiliary `τ̃_g`: library uses a simplified **obs-level** weighted mean — coincides with paper Eq. (8) for uniform within-group weights, **deviates** otherwise (PR-B reconciliation target; see the Eq. 8 deviation note above + Gaps).
...
- [ ] **R parity fixture vs `didimputation`** — none on file (PR-B deliverable).
- [ ] **Dedicated `tests/test_methodology_imputation.py`** with eq./theorem-numbered Verified Components (PR-B deliverable).
...
| Leave-one-out variance refinement | bool | (library/supplement) | Supp. App. A.9 finite-sample improvement |
...
| **Supp. App. A.9** — leave-one-out variance modification | p. 3256, 3272 | Finite-sample SE refinement |
...
- **Headline PR-B correction (surfaced by the PR-A AI review):** reconcile the auxiliary `τ̃_g` aggregator to the exact unit-clustered Eq. (8) — see the Eq. 8 deviation note above. The current obs-level weighted mean is a valid conservative simplification, not the variance-minimizing form; fix the code (`_compute_auxiliary_residuals_treated`) + its docstring + the REGISTRY label, with a methodology test on a diverging case (`aux_partition="cohort"` or non-uniform weights). Tracked in TODO.md (Methodology/Correctness).
- **For PR-B:** the highest-value missing artifact is the **R `didimputation` parity fixture** and a **`tests/test_methodology_imputation.py`** mapping each Verified Component to the equation/theorem numbers confirmed above. The `v*_it` closed form (Supp. Prop. A3) should be obtained from the Supplementary Material before formally validating the eq.-7 variance internals beyond the FE-only reconstruction the REGISTRY currently documents.
