...
- **Group-specific linear trends `DID^{fd}_{g,l}`** (Web Appendix Section 1.3, Assumption 12). Generalizes Mora-Reggio (2019) to staggered timing:
  ```
  DID^{fd}_{g,l} = Y_{g, F_g - 1 + l} - Y_{g, F_g - 2 - l} - (Y_{g, F_g - 1} - Y_{g, F_g - 2})
                  - (1 / N^g_{F_g - 1 + l}) * sum_{g' : D_{g',1} = D_{g,1}, F_{g'} > F_g - 1 + l}
                      (Y_{g', F_g - 1 + l} - Y_{g', F_g - 2 - l} - (Y_{g', F_g - 1} - Y_{g', F_g - 2}))
  ```
  Identification floor: requires `F_g >= 3` (two pre-periods to construct the baseline first-difference). **Lemma 6**: under Assumptions 3 + 12, `E[DID^{fd}_{g,l} | D] = delta_{g,l} - delta_{g,l-1}` with convention `delta_{g,0} = 0`.
...
- **Heterogeneity testing across cohorts** (Web Appendix Section 1.5; Section 4-style joint test, Lemma 3). Estimator `beta_hat^het_l` regresses `S_g * (Y_{g, F_g-1+l} - Y_{g, F_g-1})` on `X_g` and indicators for `F_g x D_{g,1} x S_g` in subsample `{g : F_g - 1 + l <= T_g}`. **Lemma 7**: `E[beta_hat^het_l | D, X] = beta^het_l` under Assumptions 1, 3, 15. Inference uses standard OLS HC robust SEs (NOT the cohort-clustered variance) - the paper notes (page 51, appendix p. 6) that this works because the regression outcome is `S_g * (Y - Y)` not `S_g * DID_{g,l}`, so the inference does not need to account for estimation noise in the DID.
...
- Treatment is **binary in the main paper**. Non-binary extensions (ordinal / continuous) live in the Web Appendix and the library's multi-horizon `DID_{g,l}` path; the per-period `DID_M` path requires binary.
...
- **`DID^{fd}_{g,l}` printed formula:** Section 3 extraction transcribes Web Appendix Section 1.3 as
  ```
  DID^{fd}_{g,l} = Y_{g, F_g - 1 + l} - Y_{g, F_g - 1 - l - 1} - (Y_{g, F_g - 1} - Y_{g, F_g - 2}) - ...
  ```
  while the Section 4 appendix extraction has
  ```
  DID^{fd}_{g,l} = Y_{g, F_g - 1 + l} - Y_{g, F_g - 1 + l - 1} - (Y_{g, F_g - 1} - Y_{g, F_g - 2}) - ...
  ```
  The Section 3 extraction notes the printed formula may be a typo; the underlying construction (a DID on first-differenced outcomes comparing `g`'s `F_g - 1 -> F_g - 1 + l` evolution to same-baseline-not-yet-switched groups) is unambiguous. Library implementation should follow Lemma 6 (`E[DID^{fd}_{g,l} | D] = delta_{g,l} - delta_{g,l-1}`) rather than verbatim-transcribing either printed form.
...
- **Revision-date discrepancy with `docs/references.rst`:** this review cites the PDF on disk as "March 2022 (revised July 2023)" because that is the revision string on the NBER cover page of `papers/w29873.pdf`. The library's citation registry `docs/references.rst:199` instead lists "(2022, revised 2024)", suggesting an even later revision exists upstream that the local PDF predates. This review reflects the 2023 revision; whether the 2024 revision substantively changes the methodology surfaces transcribed above (the six numbered assumptions, the cohort-recentered plug-in variance, the `DID_l` / `DID^n_l` / `DID_l^pl` / `DID^X` / `DID^{fd}` constructions) has NOT been verified against a 2024 PDF. Reconciliation between this review and `docs/references.rst` is deferred to the tracker-promotion PR-B (or a later PR if a 2024 PDF is fetched and re-reviewed).
