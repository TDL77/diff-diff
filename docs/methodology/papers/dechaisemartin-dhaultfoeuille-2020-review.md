...
**PDF reviewed:** papers/Chaisemartin-TwoWayFixedEffects-2020.pdf
...
- Application 1 (page 2983-2984): standard errors clustered by county.
- Application 2 (page 2985): standard errors clustered at the worker level.
- Cross-estimator t-statistics (footnotes 23-24, pages 2985-2986): "The standard errors of `beta_hat_fe - DID_M` and `beta_hat_fd - DID_M` are computed with a worker-level clustered bootstrap." So clustered bootstrap at the panel-unit level is the recommended default both for `DID_M` SEs and for differences between estimators.

No explicit weight distribution (Rademacher / Mammen / Webb), iteration count, or step-by-step algorithm is given in the main text - these conventions live in the online Appendix and the Stata package help files. The 2022/2024 NBER WP 29873 companion materializes the cohort-recentered analytical plug-in variance.
...
- [x] `DID_M^pl` single-lag placebo and longer-lag extensions.
- [x] `sigma-underbar` / `s-underbar` weight diagnostics (Corollary 1) and `twowayfeweights` parity.
- [x] Clustered bootstrap at the panel-unit level for SEs and for cross-estimator t-tests.
...
- The Phase 2 event-study path (`DID_l`, `DID_{g,l}`, `DID^{pl}_l`, normalized `DID^n_l`, cost-benefit `delta`, sup-t simultaneous bands) implements the 2022/2024 NBER WP 29873 dynamic companion - see the separate review for that paper.
...
- 6 documented deviations from R `DIDmultiplegtDYN` (see REGISTRY § ChaisemartinDHaultfoeuille for the full text and citations):
  - **Equal-cell weighting (Python) vs cell-size weighting (R, absent explicit weights):** the paper's main-text equations use observation counts `N_{g,t}` as weights. The library aggregates to cell means up-front and then weights each cell equally (which is paper-literal at the cell level but differs from R's individual-row weighting on unbalanced inputs). Parity holds exactly when each `(g,t)` cell has the same observation count, which the parity-test generators enforce.
  - **`<50%` switcher warning at far horizons** (Phase 2 dynamic-companion extension; warns when fewer than 50% of the `l=1` switchers contribute at a far horizon `l`, per the Favara-Imbs application footnote 14).
  ...
  - **SE normalization ~4% smaller than R:** documented difference in the variance denominator on cohort-recentered IF aggregation.
  - **A11 zero-retention convention:** per-period A11 violations zero-out the relevant `DID_{+,t}` / `DID_{-,t}` and emit a consolidated warning, rather than failing or silently extrapolating.
  - **Singleton-cohort degeneracy NaN handling:** singleton-baseline groups are excluded from variance only (per footnote 15 of the dynamic paper) but retained in the point estimate as period-based stable controls.
...
- **Closed-form analytical variance for `DID_M`:** Not given in the main text. The 2022/2024 NBER WP 29873 companion paper materializes the cohort-recentered plug-in variance (Web Appendix Section 3.7.3); the 2020 AER paper relies on clustered bootstrap.
