...
The paper does not prescribe a bootstrap weight distribution (Rademacher / Mammen / Webb), an iteration count, or a step-by-step algorithm for `DID_M` SE construction. The library's implementation choices (e.g., multiplier bootstrap with `n_bootstrap >= 199` on the joiner / leaver / staggered switcher cells) are library decisions, not paper-backed defaults; see the Requirements checklist below for the explicit library-vs-paper attribution. The dynamic companion (NBER WP 29873, revised July 2023) materializes the cohort-recentered analytical plug-in variance for the multi-horizon `DID_l` path; that analytical formula does not apply to the AER 2020 per-period `DID_M`.
