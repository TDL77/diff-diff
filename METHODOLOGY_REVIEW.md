...
**Verified Components:**
- [x] Eq. 3 / Theorem 1 (Design 1' WAS identification: `WAS = E[ΔY]/E[D]`) — `tests/test_methodology_had.py::TestHADTheorem1Design1Prime` (6 tests, MC recovery + N(0,1) coverage at `n_replicates=200`, G=1000)
...
1. **Equal-weighting on the continuous path** (paper does not prescribe a unit-weighting scheme; library uses per-unit `w_g = 1` matching `_nprobust_port.lprobust`'s default, NOT cell-size weights). Locked in `tests/test_methodology_had.py::TestHADDeviations::test_equal_weighting_invariant_under_cell_size_perturbation`.
